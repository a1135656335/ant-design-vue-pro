  ...
  <div>
    ...
    <s-table
      ...
      <span slot="brandName" slot-scope="text">
        <span >{{text | brandNameFilter(that)}}</span>
      </span>
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return request({
          url: `/stb/models/v1?size=${parameter.pageSize}&current=${parameter.pageNo}${queryStr}`,
          method: 'get'
        }).then(res => {
          var result = {}
  ...
  filters: {
    brandNameFilter (type, that) {
      return that.brandMaps[type]
    }
  },
  created () {
    this.tableOption()
    ...
    handleSearch () {
      request({
        ...
        if (ret.statusCode === 2000) {
          console.log('handleSearch', ret.data)
          this.brands = ret.data
          ret.data.forEach((b) => { this.brandMaps[b.id] = b.name })
        }
      })
    },
    resetSearchForm () {
